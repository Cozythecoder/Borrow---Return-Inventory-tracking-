...
$(document).ready(function () {
    ...
    var myTable = $('#datatable_equipment').DataTable({
                ...
                action: function (e, dt, node, config) {
                    ...
                    if (idsToDelete.length > 0) {
                        if (confirm('Are you sure you want to delete the selected rows?')) {
                            $.ajax({
                                url: '/delete_multiple_equipment/', // Your endpoint for batch deletion
                                type: 'POST',
                                data: {
                                    ids: idsToDelete
                                },
...
`
            ...
            },
            {
                ...
                action: function (e, dt, node, config) {
                    if (confirm('Are you sure you want to clear all data? This action cannot be undone.')) {
                        $.ajax({
                            url: '/clear_all_data_equipment/', // Your endpoint for clearing all data
                            type: 'POST',
                            success: function (response) {
                                dt.ajax.reload(); // Reload the DataTable
                            },
            ...
            },
            {
                ...
                action: function (e, dt, node, config) {
                    ...
                    link.href = '/static/csv-template_equipment.csv'; // Path to your CSV template
                    link.download = 'csv-template_equipment.csv';
                    link.click();
                }
            }
        ...
        onDeleteRow: function (alteditor, rowdata, success, error, originalrowdata) {
            ...
            $.ajax({
                url: '/delete_equipment/' + rowObject.id + '/',
                type: 'DELETE',
                contentType: 'application/json',
                data: rowdata,
                success: success,
        ...
        onEditRow: function (alteditor, rowdata, success, error, originalrowdata) {
            $.ajax({
                url: '/edit_equipment/' + rowdata.id + '/',
                type: 'PUT',
                contentType: 'application/json',
                data: JSON.stringify({
                    display_name: rowdata.display_name,
                    asset_tag: rowdata.asset_tag,
                    internal_reference: rowdata.internal_reference,
                    model: rowdata.model,
