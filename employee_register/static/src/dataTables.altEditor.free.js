            ...
            altEditor.prototype,
            {
                    ...
                    for (var j in columnDefs) {
                        if (columnDefs[j].name != null) {
                            ...
                            if (columnDefs[j].type.indexOf("select") >= 0 && columnDefs[j].select2) {
                                ...
                                $(selector).find(jquerySelector).attr('autocomplete', 'off'); // Disable autocomplete on Select2
                            }
                            // Added checkbox
                            else if (columnDefs[j].type.indexOf("checkbox") >= 0) {
                                if (this._quoteattr(selectedValue) === "true" || this._quoteattr(selectedValue) == "1") { // MS SQL Databases use bits for booleans. 1 is equivalent to true, 0 is false
                            ...
                            $(selector).find(jquerySelector)
                                ...
                                .attr('autocomplete', 'off') // Add this line to disable autocomplete
                                .trigger("change"); // required by select2
                        }
                    }
                
                    // Focus the first input field and disable its autocomplete
                    $(selector + ' input[0]').attr('autocomplete', 'off').trigger('focus'); 
                ...
                _editRowData: function () {
                    ...
                    $inputs.filter(':input[type!="file"], :input[type="checkbox"]').each(function (i) {
                        $(this).attr('autocomplete', 'off'); // Add this line to disable autocomplete
